...
public class GraphView extends SurfaceView implements SurfaceHolder.Callback {
	class GraphThread extends Thread {
		...
		// ztype: 0 = none, 1 = x, 2 = y, 3 = both
				...
				public GraphThread(SurfaceHolder surfaceHolder, Handler handler) {
					...
					tracePaint.setColor(Color.parseColor("#606060"));
					ticksPaint = new Paint();
					ticksPaint.setColor(Color.parseColor("#606060"));
					axisPaint = new Paint();
					axisPaint.setColor(Color.parseColor("#373737"));
					...
					//for (int i=0;i<NUM_FNS;i+=1) {
						fnPaint = new Paint ();
						fnPaint.setStyle (Paint.Style.FILL_AND_STROKE);
						fnPaint.setStrokeWidth (2);
						fnPaint.setAntiAlias(true);
					//}
					fnPaint.setColor (Color.parseColor("#33B5E5"));
				...
				if (graphMode == TRACE) {
					...
					yTextCur += textBot;
					//for (i=0;i<NUM_FNS;i+=1) {
						if (!appState.isCalcEmpty ()) {
							yloc = appState.getYTracePt(xloc);
							ygraph = appState.getYGraphPt (yloc);
							c.drawCircle (appState.traceLoc,ygraph,4,fnPaint);
							c.drawText ("Fn"+Integer.toString(1)+" = "+Float.toString(yloc),
	...
	public final int GRAPH=0,TRACE=1;

	public GraphView(Context context, AttributeSet attrs) {
		super(context, attrs);
		appState=SharedData.getInstance().appState;
    ...
    public void setAppState(CalculationsState state){
    	appState=state;
    }
	...
	public void surfaceCreated(SurfaceHolder holder) {
		...
		thread.start();
		fnPts = new float[(((int)getXMax ())+1)*4];
		appState.initFns ();
		appState.plotFns (fnPts);
	}
