...
private void solveModFunction(){
	...
	for(int count=0;count<tokens.size();count++){
		if(isPrimitive(tokens.get(count).toString()) && ((Primitive)tokens.get(count)).getVal()=='%'){
			containsMod=true;
			modOpIndex=count;
			break;
		}
	}
	if(containsMod && modOpIndex!=0 && (modOpIndex+1)<tokens.size() && tokens.get(modOpIndex-1)!=null && tokens.get(modOpIndex+1)!=null && tokens.get(modOpIndex-1) instanceof ComplexNumber && tokens.get(modOpIndex+1) instanceof ComplexNumber ){
		
		double num=((ComplexNumber)tokens.get(modOpIndex-1)).re;
		double den=((ComplexNumber)tokens.get(modOpIndex+1)).re;
		if(den!=0){
			double result=num%den;
			tokens.remove(modOpIndex+1);
			tokens.remove(modOpIndex);
			tokens.remove(modOpIndex-1);
			tokens.add(modOpIndex-1,new ComplexNumber(result, 0));
	...
	public int bspcHelper(int index, boolean remove,
		...
		if(tokLens.size()!=0 && (tokLens.size()-1)<toks.size()){
	...
	public void addToken(String t, int len, int index) {
		if(SharedData.getInstance().isSolvedPressed){
			index=0;
			viewIndex=0;
