...
public class ScalingUtility {
	...
	protected ScalingUtility(Activity activity) {
		...
		runningDeviceDensity= activity.getResources().getDisplayMetrics().density;
	...
	private void recursiveScaleView(View childView) {
		...
		if(childView.getLayoutParams() instanceof AbsListView.LayoutParams ){
			...
			if(linearParams!=null){
				if (linearParams.width != MarginLayoutParams.WRAP_CONTENT && linearParams.width!=-1  &&  !(tag!=null && tag.equalsIgnoreCase("constantwidth"))) {
					linearParams.width *= minScalingFactor;
				}

				if (linearParams.height != MarginLayoutParams.WRAP_CONTENT && linearParams.height !=-1  &&  !(tag!=null && tag.equalsIgnoreCase("constantheight"))) {
